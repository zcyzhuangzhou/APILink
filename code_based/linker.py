...
from base import SOPostAPILinker
from model.base import SOPost


class CodeBasedSOPostAPILinker(SOPostAPILinker):
    def __init__(self):
    ...
    def init_api_table(self):
        api_graph: GraphData = GraphData.load(str(Path(DATA_DIR) / "android27_api.graph"))
        api_nodes = api_graph.find_nodes_by_property("api_type", 2) + api_graph.find_nodes_by_property("api_type", 3)

        for api_node in api_nodes:
            qualified_name = api_node["properties"]["qualified_name"]
            # 將qualified name的处理结果保存，后面直接引用，和后面直接再次处理，那个比较合适
    ...
    def link_one(self, post: SOPost, api):
        ...
        if long_name in self.long_name_dic.keys():
            long_name_set = self.long_name_dic[api]
            score_dic = {}
        ...
        # 模糊匹配，首先匹配long name,其次匹配short name，然后是参数个数，最后是参数相似度
        if short_name in self.short_name_dic.keys():
            short_name_set = self.short_name_dic[api]
            score_dic = {}
